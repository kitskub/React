...
public class TriangleEPA {
	...
	private float mDet;
	private Vector3 mClosestPoint = Vector3.ZERO;
	...
	public Vector3 computeClosestPointOfObject(Vector3[] supportPointsOfObject) {
		...
		return p0.add(supportPointsOfObject[mIndicesVertices[1]].sub(p0).mul(mLambda1).add(supportPointsOfObject[mIndicesVertices[2]].sub(p0).mul(mLambda2)).mul(1 / mDet));
	}
